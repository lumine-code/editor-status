...
module.exports = class EditorPositionView {
  ...
  subscribeToConfig() {
    ...
    this.configSubscriptions.add(
      lumine.config.observe("editor-status.template", (value) => {
        this.templateSelection = value || "";
        this.updateTemplate();
      }),
      lumine.config.observe("editor-status.custom", (value) => {
  ...
  updateTemplate() {
    ...
    if (template && template.trim()) {
      try {
        ...
        lumine.notifications.addWarning("editor-status: invalid template", {
