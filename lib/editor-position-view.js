...
module.exports = class EditorPositionView {
  ...
  subscribeToConfig() {
    ...
    this.configSubscriptions.add(
      lumine.config.observe("item-status.template", (value) => {
        this.templateSelection = value || "";
        this.updateTemplate();
      }),
      lumine.config.observe("item-status.custom", (value) => {
  ...
  updateTemplate() {
    ...
    if (template && template.trim()) {
      try {
        ...
        lumine.notifications.addWarning("item-status: invalid template", {
