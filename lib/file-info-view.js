const { Disposable } = require("lumine");
const fs = require("@lumine-code/fs-plus");

module.exports = class FileInfoView {
  constructor() {
    this.element = document.createElement("status-bar-tile");
    this.element.classList.add("file-info");

    this.element.getActiveItem = this.getActiveItem.bind(this);

    this.activeItemSubscription = lumine.workspace.getCenter().onDidChangeActivePaneItem(() => {
      this.subscribeToActiveItem();
    });
    this.subscribeToActiveItem();

    this.registerTooltip();
    const clickHandler = (event) => {
      const isShiftClick = event.shiftKey;
      this.showCopiedTooltip(isShiftClick);
      const text = this.getActiveItemCopyText(isShiftClick);
      lumine.clipboard.write(text);
      return setTimeout(() => {
        return this.clearCopiedTooltip();
      }, 2000);
    };

    this.element.addEventListener("click", clickHandler);
    this.clickSubscription = new Disposable(() =>
      this.element.removeEventListener("click", clickHandler),
    );
  }

  registerTooltip() {
    this.tooltip = lumine.tooltips.addComposite(this.element, [
      {
        title: "Copy absolute file path",
        keyBindingExtra: "LMB",
      },
      {
        title: "Copy relative file path",
        keyBindingExtra: "Shift+LMB",
      },
    ]);
  }

  clearCopiedTooltip() {
    this.copiedTooltip?.dispose();
    this.registerTooltip();
  }

  showCopiedTooltip(copyRelativePath) {
    this.tooltip?.dispose();
    this.copiedTooltip?.dispose();
    const text = this.getActiveItemCopyText(copyRelativePath);
    this.copiedTooltip = lumine.tooltips.add(this.element, {
      title: `Copied: ${text}`,
      trigger: "manual",
      delay: {
        show: 0,
      },
    });
  }

  getActiveItemCopyText(copyRelativePath) {
    const activeItem = this.getActiveItem();
    let path = activeItem?.getPath?.();
    if (path == null) {
      return activeItem?.getTitle?.() || "";
    }

    // Make sure we try to relativize before parsing URLs.
    if (copyRelativePath) {
      const relativized = lumine.project.relativize(path);
      if (relativized !== path) {
        return relativized;
      }
    }

    // An item path could be a url, we only want to copy the `path` part
    if (path?.indexOf("://") > 0) {
      const parsed = URL.parse(path);
      if (parsed) {
        path = parsed.pathname + parsed.search;
      }
    }
    return path;
  }

  subscribeToActiveItem() {
    this.modifiedSubscription?.dispose();
    this.titleSubscription?.dispose();
    const activeItem = this.getActiveItem();

    if (activeItem) {
      if (this.updateCallback == null) {
        this.updateCallback = () => this.update();
      }

      if (typeof activeItem.onDidChangeTitle === "function") {
        this.titleSubscription = activeItem.onDidChangeTitle(this.updateCallback);
      }

      this.modifiedSubscription = activeItem.onDidChangeModified?.(this.updateCallback);
    }

    this.update();
  }

  destroy() {
    this.activeItemSubscription.dispose();
    this.titleSubscription?.dispose();
    this.modifiedSubscription?.dispose();
    this.clickSubscription?.dispose();
    this.copiedTooltip?.dispose();
    this.tooltip?.dispose();
  }

  getActiveItem() {
    return lumine.workspace.getCenter().getActivePaneItem();
  }

  update() {
    this.updatePathText();
    this.updateBufferHasModifiedText(this.getActiveItem()?.isModified?.());
  }

  updateBufferHasModifiedText(isModified) {
    if (isModified) {
      this.element.classList.add("buffer-modified");
      this.isModified = true;
    } else {
      this.element.classList.remove("buffer-modified");
      this.isModified = false;
    }
  }

  updatePathText() {
    const path = this.getActiveItem()?.getPath?.();
    const title = this.getActiveItem()?.getTitle?.();

    if (path) {
      const relativized = lumine.project.relativize(path);
      this.element.textContent = relativized != null ? fs.tildify(relativized) : path;
    } else if (title) {
      this.element.textContent = title;
    } else {
      this.element.textContent = "";
    }

    this.element.style.display = this.element.textContent === "" ? "none" : "";
  }
};
